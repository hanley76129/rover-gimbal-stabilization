...
mission_control_manager::read_gimbal_target_request()
{
  ...
  gimbal_target_request gtr = {
    .axis = static_cast<gimbal_axis>(payload[0]),
    .target_angle = static_cast<uint8_t>(payload[1]),
  };

  hal::print<64>(
    *console, "axis = 0x%X, target_angle = 0x%X", payload[0], payload[1]);
...
void mission_control_manager::reply_imu_accel_request(int16_axis accel)
{
  ...
  hal::can_message reply{ .id = static_cast<uint32_t>(
                            can_message_id::imu_accel_reply),
                          .length = 6,
...
bool mission_control_manager::reply_heartbeat(uint8_t imu_status,
                                              uint8_t lcd_status)
{
